...
class FactStore {
  ...
  async get(factId) {                               //gets a single fact by ID
    let collection = await this.collection();
    console.log('getting fact id: ', factId)
    let id = ObjectId(factId);
...
//    try {
//      cost {this.client, collection} = await connectToDatabase();
//      let result = await collection.deleteAll(entry)
//    }
//   }
// }
